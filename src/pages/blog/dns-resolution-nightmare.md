...
On August 3rd, the day after I published those updates on my platforms, I researched about domains and the Domain Name System (DNS) on Cloudflare. Apparently, to my understanding, if we set an orange-proxy in Cloudflare DNS, then Cloudflare will be able to [edge-cache them](https://developers.cloudflare.com/dns/proxy-status/#benefits) (on top of resolving the DNS), whereas if we set grey-proxy, Cloudflare will only act as a DNS resolver. It seemed interesting, so in my infinite wisdom, I decided in the morning to migrate my DNS records from Netlify to Cloudflare (both are free). Here are the steps that I did:
...
On the image above, notice the red box. At that time, I realized that all of them ended with the suffix `.imballinst.dev`. So, `imballinst.dev` got redirected to `imballinst.netlify.app.imballinst.dev`. Of course I couldn't visit the thing. So, I fixed all of them, and a few minutes later, I was able to visit this site. _Phew_, or so I thought.

There was another issue. Previously, all of the things that I hosted in my VPS via [Coolify](https://coolify.io/) used a certificate via [Traefik's DNS challenge](https://coolify.io/docs/knowledge-base/proxy/traefik/dns-challenge). So, those subdomains under Coolify would have a TLS certificate automatically applied with Let's Encrypt, no matter how "deep" the subdomain is. For example, `a.b.c.imballinst.dev` would have HTTPS available all the same. Not with Cloudflare, though. Cloudflare only provides free TLS for orange-proxied DNS [for the _first_ subdomain](https://community.cloudflare.com/t/use-a-second-level-sub-domains-with-ssl/405833/3), e.g. `atoyr.imballinst.dev` and `hello.imballinst.dev`. If I have `universe.hello.imballinst.dev`, it would not be covered by Cloudflare's TLS and as such, my browser wasn't able to visit those sites.
...
- Give up the Cloudflare orange-proxy DNS for those second-level subdomains and just use Traefik's TLS.
- Flatten the nested subdomains. For example, `universe.hello.imballinst.dev` becomes `universe-hello.imballinst.dev`.

Considering those second-level subdomains are mostly internal/testing only and ACM costs $10/month, it was an automatic no. Flattening was an interesting option, but then again, since they are internal/testing only and won't attract real traffic, it was probably fine (hopefully). So, I gave up the orange-proxy and used the grey-proxy instead.
...
- If you have nested subdomains, are using Cloudflare, AND you don't want to use Total TLS, you have to switch to grey-proxy or flatten the nested subdomains, because the orange-proxy won't work on nested subdomains.
